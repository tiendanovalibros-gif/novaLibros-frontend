...
export default function TiendaCrudPanel({ tiendaSeleccionada, onSeleccionar }: TiendaCrudPanelProps) {
  ...
  const ciudadesDisponibles = useMemo(() => {
    ...
    if (
      form.ciudad &&
      !base.some(c => normalizeUbicacionTexto(c) === normalizeUbicacionTexto(form.ciudad))
    ) {
  ...
  const abrirEditar = (tienda: Tienda) => {
    const dep = tienda.ciudad ? encontrarDepartamentoPorCiudad(tienda.ciudad) : null;
    setTiendaActual(tienda);
    setForm({
      nombre: tienda.nombre,
      direccion: tienda.direccion,
      departamento: dep ?? (tienda.ciudad ? DEPARTAMENTO_OTROS : ""),
  ...
  const validarFormulario = (): boolean => {
    ...
    if (form.nombre.trim().length < 3) { setFormError("El nombre debe tener al menos 3 caracteres"); return false; }
    if (!form.direccion.trim()) { setFormError("La dirección es obligatoria"); return false; }
    if (!form.departamento.trim()) { setFormError("El departamento es obligatorio"); return false; }
    if (!form.ciudad.trim()) { setFormError("La ciudad es obligatoria"); return false; }
    return true;
  };
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          {DEPARTAMENTOS_ORDENADOS.map(dep => (
      ...
      {!readOnly && (
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-500">Comprueba dirección + ciudad antes de guardar.</span>
        </div>
      )}
      {validationResult && (
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h2 className="text-slate-800 font-bold text-base">Tiendas ({tiendas.length})</h2>
        <button
          onClick={abrirCrear}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 rounded-lg text-xs font-semibold text-white hover:bg-blue-700 transition-colors"
        >
          <PlusIcon />
          Agregar
        </button>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <div className="py-12 text-center text-slate-500 text-sm">Cargando tiendas...</div>
        ) : error ? (
          <div className="py-12 text-center">
            <p className="text-red-600 font-semibold mb-2">{error}</p>
            <button onClick={cargarTiendas} className="text-blue-600 text-sm font-medium hover:underline">
              Reintentar
            </button>
          </div>
          ...
          <div className="py-12 text-center text-slate-500">
            ...
            <p className="font-semibold text-slate-700 text-sm">No hay tiendas</p>
            <p className="text-xs mt-1">Agrega una nueva tienda para comenzar</p>
          </div>
        ) : (
          <ul className="divide-y divide-slate-100">
            {tiendasFiltradas.map(tienda => (
              <li
                ...
                className={`flex items-start justify-between gap-2 px-3 py-3 rounded-lg cursor-pointer transition-colors ${
                  tiendaSeleccionada?.id === tienda.id
                    ? "bg-blue-50 border border-blue-200"
                    : "hover:bg-slate-50"
                }`}
              >
                <div className="min-w-0">
                  <p className="text-slate-900 text-sm font-semibold truncate">{tienda.nombre}</p>
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    onClick={e => { e.stopPropagation(); abrirEditar(tienda); }}
                    title="Editar"
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); abrirEliminar(tienda); }}
                    title="Eliminar"
      ...
      {dialogMode === "create" && (
        <Modal title="Agregar tienda" onClose={cerrarDialogo}>
          ...
          <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-slate-100">
            <button onClick={cerrarDialogo} className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
              Cancelar
            </button>
            <button onClick={handleCrear} disabled={saving} className="px-5 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-60 transition-colors">
      ...
      {dialogMode === "edit" && tiendaActual && (
        <Modal title={`Editar: ${tiendaActual.nombre}`} onClose={cerrarDialogo}>
          ...
          <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-slate-100">
            ...
            <button onClick={handleEditar} disabled={saving} className="px-5 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-60 transition-colors">
      ...
      {dialogMode === "delete" && tiendaActual && (
        <Modal title="Eliminar tienda" onClose={cerrarDialogo}>
          ...
          <div className="flex gap-3 justify-center mt-2">
            <button onClick={cerrarDialogo} className="px-5 py-2 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
              Cancelar
            </button>
            <button onClick={handleEliminar} disabled={saving} className="px-5 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 disabled:opacity-60 transition-colors">
